...
class PPOTrainer:
    def __init__(self, env, actor_model, critic_model, actor_optimizer, critic_optimizer, gamma=0.99, clip_param=0.2):
        self.env = env
        ...
        self.old_actor_model = ActorNetwork(env.observation_space.shape[0], env.action_space.shape[0]).to(device)
    ...
    def train(self, num_episodes=5000):
        for episode in trange(num_episodes):
            ...
            while not done and not truncated:
                ...
                with torch.no_grad():
                    old_mean, old_logstd = self.old_actor_model(state)
                    old_std = torch.exp(old_logstd)
                    old_dist = Normal(old_mean, old_std)
                    old_log_prob = old_dist.log_prob(action_clamped).sum()
...
def main():
    wandb.init(project="rl-pendulum-ppo")
    ...
    old_actor_model = ActorNetwork(state_dim, action_dim).to(device)
    old_actor_model.load_state_dict(actor_model.state_dict())

    actor_optimizer = optim.Adam(actor_model.parameters(), lr=2e-4)
    critic_optimizer = optim.Adam(critic_model.parameters(), lr=2e-4)

    trainer = PPOTrainer(env, actor_model, critic_model, actor_optimizer, critic_optimizer, gamma=0.99, clip_param=0.2)
    trainer.train()

    torch.save(actor_model.state_dict(), "actor_model_ppo.pth")
    torch.save(critic_model.state_dict(), "critic_model_ppo.pth")
    test(actor_model)
